...
function fileToObject(xitString) {
    ...
    // TODO -> finish impl, change structure a bit. Should be a map of the possible types with their contents.
    const parseXitModifiers = (content) => {
        return content.split(/\s+/).map((token) => {
            if (token.match(xitLineModifierPatterns.priority)) {
                return {
                    type: PRIORITY_MOD_TYPE
                }
            }
        });
    };
        ...
        xitObject.lines.push(
            {
                ...
                modifiers: parseXitModifiers(trimmedContent)
            }
        );
    };
